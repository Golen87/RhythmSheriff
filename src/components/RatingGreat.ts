...
export default class RatingGreat extends Phaser.GameObjects.Container {
	private iconGlow: Phaser.GameObjects.Image;
	private icon: Phaser.GameObjects.Image;
	private starGlow: Phaser.GameObjects.Image[];
	private starOutline: Phaser.GameObjects.Image[];
	private starColor: Phaser.GameObjects.Image[];
	private starInside: Phaser.GameObjects.Image[];
	private sparks: { image: Phaser.GameObjects.Image; offset: number }[];
	...
	constructor(scene: BaseScene, x: number, y: number) {
		...
		for (var i = 0; i < 2; i++) {
			...
			this.starGlow[i] = scene.add.image(
				sx + glowOffset.x,
				sy + glowOffset.y,
				"superb_star_glow"
			);
			...
			this.starInside[i] = scene.make.image({
				x: x + sx * this.scaleX,
				y: y + sy * this.scaleY,
				key: "superb_star_inside",
				add: false,
			});
			this.starInside[i].setScale(this.scaleX);
			this.starColor[i].mask = new Phaser.Display.Masks.BitmapMask(
				scene,
				this.starInside[i]
			);
		}
		...
		for (var i = 0; i < sparkPos.length; i++) {
			...
			let image = scene.add.image(sx, sy, key);
			image.setScale(0.9);
			this.add(image);
	...
	update(time: number, delta: number) {
		...
		for (var i = 0; i < this.sparks.length; i++) {
			let { image, offset } = this.sparks[i];

			let t = (-3 * time) / 1000 + offset;
